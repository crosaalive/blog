...
<div>
	...
	<div class="right">
		...
		<br><br><br>
<div class="article">
	<form action="<?php echo U('Admin/Article/editHandle');?>" enctype="multipart/form-data" method="post">文章标题<input type="text" style="width:597px;" name="title" value="<?php echo ($content['title']); ?>">
		<br>	
		<br>	
		所属分类
		<select name="cat_id" id="cat_id">
			<option value="">......请选择......</option>
			<?php if(is_array($list)): foreach($list as $key=>$v): ?><option value="<?php echo ($v['cid']); ?>"
				<?php if($v['cid'] == $content['cat_id']): ?>selected="selected"<?php endif; ?>>
					<?php echo ($v['cat_name']); ?></option><?php endforeach; endif; ?>
			</select>
			&nbsp;&nbsp;是否在前台页面显示
			<input name="is_show" type="checkbox" value="1" <?php if($content['is_show'] == 1 ): ?>checked="checked"<?php endif; ?> /><br><br>
			<p>文章缩略图 :　<input type="file" name="thumb" /> </p>
			<input type="hidden" name="cat_name" id="cat_name" value="<?php echo $content['cat_name']; ?>">	
			<br>	
			<br>	
			<textarea id="editor_id" name="content" style="width:600px;height:400px;">
				<?php if($content['id'] > 0 ): echo $content['content']; ?>	
					<?php else: ?>	
					&lt;strong&gt; 文章内容&lt;/strong&gt;<?php endif; ?>
			</textarea>
			<br>	
			<?php if($content['id'] > 0 ): ?><input type="hidden" name="edit" value="1">
				<input type="hidden" name="id" value="<?php echo $content['id']; ?>">		
				<input type="submit" value="提交修改">	
				<?php else: ?>	

				<input type="submit" value="发布文章"><?php endif; ?>
		</form>
</div>
	</div>
</div>
