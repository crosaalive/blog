...
<br><br><br>
<div class="cat_form">
<form action="<?php echo U('Admin/Category/formHandle');?>" method="post" onsubmit="return check(this)">
  所属分类
  <select name="parent_id" id="">
    <option value="">......请选择......</option>
    <?php if(is_array($list)): foreach($list as $key=>$v): if($data['cid'] != $v['cid'] ): ?><option value="<?php echo ($v['cid']); ?>"<?php if($data['parent_id'] == $v['cid']): ?>selected="selected"<?php endif; ?>>
          <?php echo ($v['cat_name']); ?></option><?php endif; endforeach; endif; ?>
    </select>
    <br>
    <br>
    分类名称
    <input type="text" name="cat_name" id="name" value="<?php echo ($data['cat_name']); ?>
    ">   [中文_字母_数字 等...]
    <br>
    <br>
    分类排序
    <input type="text" name="sort" id="" value="<?php echo ($data['sort']); ?>
    ">  [0-999]
    <br>
    <br>
    <input type="hidden" name="sid" value="<?php echo ($data['cid']); ?>
    ">
    <input type="submit" class="submit" value="确认修改"></form>
  </div>
	</div>
</div>
